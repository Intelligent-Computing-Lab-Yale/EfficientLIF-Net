...
class VGG16_layershare(nn.Module):
    def __init__(
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for t in range(self.total_timestep):
            ...
            out = self.lif3(self.bn4(self.conv4(out)))
            out = self.maxpool2(out)

            out = self.lif5(self.bn5(self.conv5(out)))
            out = self.lif5(self.bn6(self.conv6(out)))
            out = self.lif5(self.bn7(self.conv7(out)))
            out = self.maxpool3(out)

            out = self.lif8(self.bn8(self.conv8(out)))
            out = self.lif8(self.bn9(self.conv9(out)))
            out = self.lif8(self.bn10(self.conv10(out)))
            out = self.maxpool4(out)

            out = self.lif11(self.bn11(self.conv11(out)))
            out = self.lif11(self.bn12(self.conv12(out)))
            out = self.lif11(self.bn13(self.conv13(out)))
